...
then
	...
	?distribution dcat:downloadURL ?file .
      ...
      RESULT=`curl --data-urlencode query="$QUERY" -H 'accept:text/tab-separated-values' https://databus.dbpedia.org/sparql | sed 's/[\"<>]//g' | grep "^http" | head -n 1`
      RESULT=`echo $RESULT | sed 's/^http:/https:/'`
      echo $RESULT
      curl -LO $RESULT
